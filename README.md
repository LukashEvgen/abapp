...
Оновлення дизайну (`MYA-20`) завершено. Додаток перейшов на канонічну брендову палітру з бізнес-карти: Pantone 7459 C teal (`#2A8FA8`) як primary, Pantone 444 C grey (`#7C8084`) як secondary, світла тема за замовчуванням. Стару золоту палітру V1 (`colors.gold`) повністю видалено. `design/tokens.json` та `src/utils/theme.ts` — єдині джерела правди для кольорів і типографіки. Референсні мокапи `design-new/` інтегровані та перевірені; актуальний код використовує сучасніший API-шар (TanStack Query, розділені сервіси) із збереженням UX/структури мокапів.
